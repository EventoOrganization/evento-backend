  ...
  io.on("connection", (socket) => {
    ...
    socket.on("create_group", async function (data) {
      try {
        ...
        let foundResponse = await Models.groupChatModel.findOne({
          _id: response._id,
        });
        let success_msg = {
      ...
      try {
        ...
        for (const userId of userIds) {
          if (typeof userId === "string") {
            ...
            const groupMessages = await Models.message
              .find({
                groupId: userId,
              })
    ...
    socket.on("send_message", async function (data) {
      ...
      try {
        if (data.groupId) {
          ...
          let receiverId = data.groupUserIds.filter(
          ...
          if (checkChatConstant) {
            ...
            const getMsg = await Models.message
              .findOne({
            ...
            const getGroupDetail = await Models.message
              .findOne({
                senderId: saveMsg.senderId,
                groupUserIds: { $in: saveMsg.groupUserIds }, // Use $in to match any of the values in the array
                _id: saveMsg._id,
    ...
    socket.on("rename_group_name", async function (data) {
      try {
        ...
        let reponse = await Models.groupChatModel.updateOne(
          criteria,
          objToSave,
        );
        let success_msg = {
    ...
    socket.on("remove_from_group", async function (data) {
      try {
        ...
        if (exits.admin.toString() == data.removerId.toString()) {
          ...
          let response = await Models.groupChatModel.findByIdAndUpdate(
            { _id: exits._id },
            { $set: { ...objToUpdate } },
          );
          let success_msg = {
    ...
    socket.on("read_notification", async (data) => {
      try {
        await Models.eventNotificationModel.updateOne(
          { _id: data._id },
          { $set: { is_read: 1 } },
        );
        await Models.followingNotificationModel.updateOne(
          { _id: data._id },
          { $set: { is_read: 1 } },
        );
        let response = await Models.eventNotificationModel.find({
          _id: data._id,
        });
        let result = await Models.followingNotificationModel.find({
          _id: data._id,
        });
        let finalResult = response ? response : result;
        let get_socket_id = await Models.socketuser.find({
          userId: data.userId,
        });
        socket.emit("read_notification", finalResult);
        io.to(get_socket_id.socketId).emit("read_notification", finalResult);
      } catch (error) {
        throw error;
      }
    });
    socket.on("unread_notification_count", async (data) => {
      try {
        let eventNotification =
          await Models.eventNotificationModel.countDocuments({
            reciverId: data.userId,
            is_read: 0,
          });
        let followingNotification =
          await Models.followingNotificationModel.countDocuments({
            reciverId: data.userId,
            is_read: 0,
          });
        let unreadCount =
          parseInt(eventNotification) + parseInt(followingNotification);
        let get_socket_id = await Models.socketuser.find({
          userId: data.userId,
        });
        socket.emit("unread_notification_count", { unreadCount: unreadCount });
        io.to(get_socket_id.socketId).emit("unread_notification_count", {
