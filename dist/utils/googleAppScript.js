...
const GOOGLE_SCRIPT_URL = "https://script.google.com/macros/s/AKfycbwVUiR4sFvY-O6qApqkco_ul-gCDXBmOhcdbAiL3h8YFdiZtHqKJAPGXPb-_CPI8BX-Qw/exec";
const GOOGLE_SCRIPT_UPDATE_URL = "https://script.google.com/macros/s/AKfycbx3PF9vVEADP-rHk-7LK7FRh4SLKY6cE2anA8q1hAWihXQw4wWnyqni-5nIbMbNYAnGYA/exec";
...
const updateGoogleSheetForEvent = async (event, action, options = {}) => {
    ...
    const formattedTimestamp = formatTimestampForSheet(new Date(), event.details?.timeZone || "UTC");
    ...
    switch (action) {
        ...
        case "updateStatus": {
            ...
            const isGuest = event.guests?.some((id) => id.toString() === user._id.toString()) ||
                event.tempGuests?.some((id) => id.toString() === user._id.toString());
            const enrichedAnswers = (eventStatus.rsvpAnswers || []).map((ans) => {
                const matched = event.questions.find((q) => q._id?.toString() === ans.questionId?.toString());
                return {
                    question: matched?.question || `[Question inconnue: ${ans.questionId}]`,
                    answer: ans.answer,
                };
            });
            payload = {
