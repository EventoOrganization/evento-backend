...
const sendEventInviteEmail = async (user, guest, event, eventLink, isCoHost = false) => {
    ...
    const params = {
        ...
        Message: {
            Subject: {
                Data: `You're Invited to ${event.title} by ${user.username || "Evento"}`,
            },
            Body: {
                Html: {
                    Data: `
            <div style="color: #333; font-family: Arial, sans-serif; font-size: 16px; line-height: 1.4; padding: 20px; background-color: #f9f9f9; display: flex; flex-direction: column; align-items: center; gap: 10px;">
              ...
              <p style="margin: 0;">You are invited to ${isCoHost ? "co-host" : "join"} the event by <strong>${user.username || "on Evento"}</strong>.</p>
...
const sendBirthdayEmail = async (follower, birthdayPerson) => {
    const params = {
        ...
        Message: {
            Subject: {
                Data: `It's ${birthdayPerson.username || "your friend's"} birthday today!`,
            },
            Body: {
                Html: {
                    Data: `
            <h3>Happy Birthday to ${birthdayPerson.username || "Your Friend"}!</h3>
            <p>Don't forget to wish <strong>${birthdayPerson.username || "your friend"}</strong> a happy birthday today!</p>
          `,
                },
            },
        },
...
const sendFeedbackEmail = async (user, feedback) => {
    ...
    try {
        const result = await ses.sendEmail(params).promise();
        console.log(`Feedback email sent successfully to help@evento-app.io`, result);
    }
    catch (error) {
