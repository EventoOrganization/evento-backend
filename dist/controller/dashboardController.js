...
module.exports = {
    ...
    UpdateProfile: async (req, res) => {
        try {
            ...
            if (req.files && req.files.profileImage) {
                ...
                req.files.profileImage.mv(process.cwd() + "/public/images/" + fileImage, function (err) {
                    if (err)
                        console.log(err);
                });
            }
            ...
            const fetchProfile = await user.findOneAndUpdate({ _id: req.session.user._id }, { $set: updateFields }, { new: true });
            console.log(req.session.user._id);
            const result = await user.findOne({ _id: req.session.user._id });
            console.log("🚀 ~ file: dashboardController.js:123 ~ UpdateProfile: ~ result:", result);
        ...
        try {
            ...
            if (userObj) {
                ...
                if (isPasswordValid) {
                    ...
                    let create = await user.updateOne({ _id: userObj._id }, {
    ...
    addNewInterest: async (req, res) => {
        try {
            ...
            if (req.files && req.files.image) {
                fileImageUrl = await fileUploadLarge(req.files.image, `interests/${newInterest._id}`);
    ...
    EditInterest: async (req, res) => {
        try {
            ...
            if (req.files && req.files.image) {
                ...
                const oldImageKey = interestData.image.split(`${process.env.S3_BUCKET_NAME}.s3.${process.env.AWS_REGION}.amazonaws.com/`)[1];
                if (oldImageKey) {
                    await deleteFileFromS3(oldImageKey);
                }
                // Téléverser la nouvelle image sur S3
                fileImageUrl = await fileUploadLarge(req.files.image, `interests/${req.body.id}`);
            }
            ...
            await interest.findOneAndUpdate({ _id: req.body.id }, { $set: updateFields }, { new: true });
    ...
    deleteInterest: async (req, res) => {
        try {
            ...
            const key = imageUrl.split(`${process.env.S3_BUCKET_NAME}.s3.${process.env.AWS_REGION}.amazonaws.com/`)[1]; // Extraire le chemin du fichier
            if (key) {
    ...
    saveSubInterest: async (req, res) => {
        try {
            ...
            if (req.files && req.files.image) {
                imageUrl = await fileUploadLarge(req.files.image, `interests/${interestId}/subInterests/${newSubInterest._id}`);
    ...
    EditSubInterest: async (req, res) => {
        try {
            ...
            if (req.files && req.files.image) {
                // Supprimer l'ancienne image de S3
                const oldImageKey = subInterestData.image.split(`${process.env.S3_BUCKET_NAME}.s3.${process.env.AWS_REGION}.amazonaws.com/`)[1];
                if (oldImageKey) {
                    await deleteFileFromS3(oldImageKey);
                }
                // Téléverser la nouvelle image sur S3
                fileImageUrl = await fileUploadLarge(req.files.image, `interests/${subInterestData.interest}/subInterests/${req.body.id}`);
            }
            // Préparer les champs à mettre à jour
            const updateFields = {
                name: name,
            ...
            await SubInterest.findOneAndUpdate({ _id: req.body.id }, { $set: updateFields }, { new: true });
    ...
    deleteEvent: async (req, res) => {
        try {
            ...
            console.log("🚀 ~ file: dashboardController.js:322 ~ deleteuser: ~ deldata:", deldata);
            res.json(1);
        }
        catch (error) {
            console.log(error);
    ...
    updateUser: async (req, res) => {
        try {
            ...
            await user.findOneAndUpdate({ _id: req.body.id }, { $set: updateFields }, { new: true });
    ...
    deleteuser: async (req, res) => {
        try {
            ...
            await Models.coHostModel.updateMany({ cohost_id: req.body.id }, { $pull: { cohost_id: req.body.id } });
