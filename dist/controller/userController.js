...
const { sendBirthdayEmail, sendEventReminderEmail, } = require("../helper/mailjetEmailService");
...
module.exports = {
    ...
    login: async (req, res) => {
        ...
        try {
            ...
            if (!logData) {
                const errorMessage = "Sorry, we don’t recognize your user. Please create an account.";
                console.log(errorMessage);
                return helper.failed(res, errorMessage);
            }
            if (logData.is_block == 1) {
                const errorMessage = "Your account is blocked by admin. Please contact the admin.";
                console.log(errorMessage);
                return helper.failed(res, errorMessage);
            }
            const checkPassword = await bcrypt.compare(v.inputs.password, logData.password);
            if (!checkPassword) {
    ...
    updateProfile: async (req, res) => {
        try {
            ...
            if (isNameExist) {
                return helper.failed(res, "Sorry this username already exists, please choose another one");
            }
            if (req.body && req.body.interest && req.body.interest.length > 0) {
                objToSave.interest = JSON.parse(req.body.interest);
                await Models.userModel.updateOne({ _id: req.user._id }, { $set: { interest: [] } });
            }
        ...
        try {
            ...
            if (isEmailExist) {
                if (req.body.otp == isEmailExist.otp) {
                    ...
                    const userDetail = await Models.userModel.findOne({ _id: isEmailExist._id }, {
                        _id: 1,
                        name: 1,
                        email: 1,
                        countryCode: 1,
                    ...
                    return await helper.success(res, "Verify OTP successfully", userDetail);
                }
                else {
    ...
    resendOtp: async (req, res) => {
        try {
            ...
            let update_otp = await Models.userModel.findOneAndUpdate({ email: req.body.email }, {
                $set: {
                    otp: otp,
                },
            }, { new: true });
    ...
    getProfile: async (req, res) => {
        try {
            ...
            const filteredPastEvents = filteredEventsPast.filter((event) => {
                ...
                const coHostFound = event.coHosts.some((coHost) => coHost._id.toString() == ID);
            ...
            const filteredPastEventsAttended = filteredEventsPast55.filter((event) => {
                const endDate = new Date(event.details.endDate); // Assuming details.endDate is a Date object
                const endTimeStr = event.details.endTime; // Assuming details.endTime is a time string in HH:mm AM/PM format
                const endTime = moment(endTimeStr, "h:mm A").format("HH:mm");
                const eventDateTime = moment(endDate)
            ...
            const filteredUpcomingEventsAttened = filteredEventsPast55.filter((event) => {
                const endDate = new Date(event.details.endDate); // Assuming details.endDate is a Date object
                const endTimeStr = event.details.endTime; // Assuming details.endTime is a time string in HH:mm AM/PM format
                if (!endTimeStr) {
                    return true;
            ...
            let countTotalEventIAttended = await Models.eventAttendesUserModel.countDocuments({
                userId: req.user._id,
            });
            let obj = {};
    ...
    getProfileByUserId: async (req, res) => {
        try {
            const userInfo = await Models.userModel
                .findOne({ _id: req.user._id })
                .select("firstName lastName name email countryCode phoneNumber address DOB profileImage deviceToken deviceType bio aboutMe role is_block is_otp_verify interest")
    ...
    createEventAndRSVPform: async (req, res) => {
        ...
        try {
            ...
            const { name, email, firstName, lastName, attendEvent, questions, additionalField, timeSlots, } = req.body;
            ...
            if (req.files) {
                const mediaKeys = Object.keys(req.files).filter((key) => key.startsWith("media"));
            ...
            if (req.files && req.files.video) {
                ...
                if (req.body.type == 2) {
                    try {
                        ...
                        console.error("Error during thumbnail generation or upload:", error);
            ...
            if (req.body.createRSVP && req.body.createRSVP == "true") {
                try {
                    ...
                    console.error("Error parsing RSVP questions or additional fields:", error.message);
                    return res.status(400).json({
                        status: false,
            ...
            if (req.body.createRSVP && req.body.createRSVP == "true") {
                try {
                    ...
                    console.error("Error parsing RSVP questions or additional fields:", error);
                    return res.status(400).json({
                        status: false,
                        message: "Invalid JSON format in RSVP data",
                    });
            ...
            if ((req.body.eventType == "private" || req.body.eventType == "public") &&
    ...
    updateEvent: async (req, res) => {
        try {
            ...
            const objToUpdate = {
                ...
                details: {
                    name: req.body.name,
                    video: req.files && req.files.video ? videoName : findEvent.details.video,
                    thumbnailVideo: req.body.type == 2 && req.files
                        ? thumbnail
            ...
            const sendNotifications = async (userIds, notificationTo) => {
                ...
                if (req.body && req.body.name) {
                    let c = await Models.eventNotificationModel.updateMany({ eventId: req.body._id }, {
                        $set: {
    ...
    getEventById: async (req, res) => {
        try {
                ...
                shareAccess =
                    virtualEvent?.user?._id.toString() === req.user._id.toString() ||
                        (virtualEvent?.guestsAllowFriend === true &&
                            virtualEvent?.guests.some((guest) => guest._id.toString() === req?.user?._id.toString()));
            ...
            const updateAccess = virtualEvent?.user?._id.toString() === req.user?._id.toString();
            ...
            var n = newTime / 1000;
            newTime = Math.floor(n);
            let isPast = newTime > moment(eventDateTime, "YYYY-MM-DD HH:mm:ss").unix();
    ...
    addCoHostAndGuestInExistingEvent: async (req, res) => {
        try {
            ...
            if (groupId) {
                Models.groupChatModel.updateOne({ eventId: req.body.eventId }, {
                    $addToSet: {
    ...
    removeGuestFromEvent: async (req, res) => {
        try {
            await Models.eventModel.findByIdAndUpdate(req.body.eventId, { $pull: { guests: req.body.guestId } }, { new: true });
    ...
    allAndVirtualEventAndNear: async (req, res) => {
        console.log("🚀 ~ file: userController.js:allAndVirtualEventAndNear ~ req.body:", req.body);
    ...
    createRSVPSubmission: async (req, res) => {
        try {
            ...
            if ((req.body.attendEvent && req.body.attendEvent == 1) ||
    ...
    myUpcomingEvents: async (req, res) => {
        try {
            ...
            const filteredUpcomingEvents = upcomingEvents1.filter((event) => {
                let ID = req.user._id.toString();
                const coHostIds = event.eventId?.coHosts.map((coHost) => coHost.toString());
                if (coHostIds.includes(ID)) {
    ...
    getProfileUser: async (req, res) => {
        try {
            const currentDate = new Date();
            const currentTime = moment(currentDate).format("HH:mm");
            let userInfo = await Models.userModel.findOne({ _id: req.params.id }, {
                _id: 1,
                name: 1,
                email: 1,
                countryCode: 1,
    ...
    uploadPhotoVideo: async (req, res) => {
        try {
            ...
            var exitsUser = checkUserGuestsOrNot != null
                ? checkUserGuestsOrNot.guests.includes(req.user._id) ||
                    checkUserGuestsOrNot.coHosts.includes(req.user._id)
                : false;
            if (exitsUser) {
                    ...
                    for (const video of uploadedvideos) {
                        ...
                        await new Promise((resolve, reject) => {
                            ffmpeg(videoPath)
                                ...
                                const thumbnailUrl = await uploadFileToS3(thumbnailPath, thumbnailName);
        ...
        try {
            ...
            if (!findData) {
                ...
                if (checkGroupHas) {
                    ...
                    await Models.groupChatModel.findByIdAndUpdate(checkGroupHas._id, update, { new: true });
                }
                ...
                if (checkGroupHas) {
                    await Models.groupChatModel.findByIdAndUpdate(checkGroupHas._id, { $pull: { users: req.user._id } }, // Changed data.userId to req.user._id
                    { new: true });
                }
    ...
    followStatusForUsersYouFollow: async (req, res) => {
        try {
            ...
            const followingMap = new Map(followingList.map((f) => [f.following.toString(), true]));
            const followersMap = new Map(followersList.map((f) => [f.follower.toString(), true]));
            const resultList = userList.map((user) => {
                const userId = user._id.toString();
                const matchingInterests = loggedInUserInterests.filter((i) => user.interests.includes(i)).length;
                return {
                    ...user.toObject(),
    ...
    userListWithFollowingStatusWithPagination: async (req, res) => {
        try {
            ...
            if (id) {
                ...
                followingMap = new Map(followingList.map((f) => [f.following.toString(), true]));
                followersMap = new Map(followersList.map((f) => [f.follower.toString(), true]));
            }
        ...
        try {
            ...
            const resultList = userList.map((user) => {
                ...
                const userInterests = Array.isArray(user.interests)
                    ? user.interests
                    : [];
                const matchingInterests = loggedInUserInterests.filter((i) => userInterests.includes(i)).length;
                return {
                    ...userObj,
    ...
    followStatusForAllUsersWithUserId: async (req, res) => {
        try {
            ...
            const resultList = userList
                ...
                .filter((result) => result.status === "follow-each-other" ||
    ...
    saveMessage: async (req, res) => {
        try {
            ...
            if (checkChatConstant) {
                ...
                await Models.chatconstant.updateOne({ _id: checkChatConstant._id }, { lastmessage: saveMsg._id });
                ...
                await Models.chatconstant.updateOne({ _id: createChatConstant._id }, { lastmessage: saveMsg._id });
                let getMsg = await Models.message
                    .findOne({
                    senderId: saveMsg.senderId,
                    reciverId: saveMsg.reciverId,
    ...
    onWholeApplicationSearch: async (req, res) => {
        try {
            ...
            const [userResults, eventResults, faqResults, rsvpSubmissionAnswerResult, contactUsResult, interestResult,] = await Promise.all([
    ...
    notificationList: async (req, res) => {
        try {
            ...
            const notificationsWithRSVP = await Promise.all(eventNotifications.map(async (notification) => {
