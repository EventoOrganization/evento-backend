...
exports.fetchConversations = async (req, res) => {
    try {
        const rawConvs = await Models.conversationModel
            ...
            .lean();
        const activeConvs = (await Promise.all(rawConvs.map(async (conv) => {
            if (!conv.eventId) {
                return conv;
            }
        ...
        const populatedConvs = await Promise.all(activeConvs.map(async (c) => {
        ...
        const withMessages = await Promise.all(populatedConvs.map(async (c) => {
...
exports.joinConversation = async (req, res) => {
    try {
        ...
        const populated = await conv.populate("participants", "username profileImage");
...
exports.leaveConversation = async (req, res) => {
    ...
    conv.participants = conv.participants.filter((id) => id.toString() !== userId.toString());
    await conv.save();
...
exports.createMessages = async (req, res) => {
    ...
    try {
        ...
        const conversation = await Models.conversationModel.findById(conversationId);
        if (conversation) {
            const update = {
            ...
            await Models.conversationModel.updateOne({ _id: conversationId }, { $set: update });
        }
        // Notifier en socket
