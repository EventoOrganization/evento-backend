...
exports.getLoggedUserProfile = async (req, res) => {
    try {
        ...
        const userInfo = await User.findOne({ _id: req.user._id })
            .select("pwaNotification pwaSubscriptions firstName lastName username email email_verified countryCode phoneNumber phone_verified address bio URL DOB profileImage interests socialLinks role devices preferences")
        ...
        const followingUsers = await Models.userFollowModel
            ...
            .select("following")
            .exec();
        const followingUserIds = followingUsers.map((follow) => follow.following.toString());
        ...
        for (const followingId of followingUserIds) {
            ...
            if (user) {
                ...
                console.warn(`User with ID ${followingId} does not exist in the database. It might be deleted but has residual follow status.`);
        ...
        const followerUsers = await Models.userFollowModel
            ...
            .select("follower")
            .exec();
        const followerUserIds = followerUsers.map((follow) => follow.follower.toString());
        const countTotalEventIAttended = await Models.eventStatusSchema.countDocuments({
            userId: req.user._id,
            status: "isGoing",
        });
...
exports.getUserProfileById = async (req, res) => {
    ...
    try {
        ...
        const userInfo = await User.findById(userId)
            .select("firstName lastName username email profileImage bio URL socialLinks interests address")
            .populate("interests", "_id name")
            .exec();
        if (!userInfo) {
            return res
        ...
        const enrichedEvents = allEvents.map((event) => {
            ...
            const isCoHost = event.coHosts.some((coHost) => coHost.userId?._id?.toString() === userId.toString());
            return {
                ...event._doc,
                isHosted,
                isCoHost,
        ...
        const isUpcomingOrOngoing = (event) => new Date(event.details.date) > today ||
            (new Date(event.details.date) <= today &&
                new Date(event.details.endDate) > today);
        const upcomingEvents = enrichedEvents.filter((event) => isUpcomingOrOngoing(event) && event.isGoing);
        const pastEventsGoing = enrichedEvents.filter((event) => new Date(event.details.endDate) < today && event.isGoing);
        const pastEventsHosted = enrichedEvents.filter((event) => new Date(event.details.endDate) < today && event.isHosted);
        return res.status(200).json({
            status: true,
            ...
            data: {
                ...
                hostedEvents: enrichedEvents
                    .filter((event) => (event.isHosted || event.isCoHost) && isUpcomingOrOngoing(event))
...
exports.updateProfile = async (req, res) => {
    ...
    try {
        ...
        let updateData = {};
        const { username, firstName, lastName, address, bio, URL, DOB, email, countryCode, phoneNumber, password, pwaSubscription, browser, pwaNotification, } = req.body;
        ...
        if ((countryCode && !phoneNumber) || (!countryCode && phoneNumber)) {
            console.log("Error: countryCode and phoneNumber must be provided together.");
            // return res.status(400).json({
            //   status: false,
        ...
        if (req.body.socialLinks) {
            ...
            if (validSocialLinks.length === 0) {
                ...
                const uniqueLinks = Array.from(new Set(validSocialLinks.map((link) => link.platform))).map((platform) => validSocialLinks.find((link) => link.platform === platform));
        ...
        if (pwaSubscription === null) {
            ...
            const subscriptionIndex = user.pwaSubscriptions?.findIndex((sub) => sub.browser === browser);
            ...
            const existingSubscription = user.pwaSubscriptions?.find((sub) => sub.browser === browser);
