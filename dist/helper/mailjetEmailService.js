...
const sendEventInviteEmail = async (user, guest, event, eventLink, isCoHost = false) => {
    if (!user || !guest || !event || !eventLink) {
        console.error("Missing required data for sending the email.");
        return;
    }
    const eventLinkWithQuery = `${eventLink}?email=${encodeURIComponent(guest.email)}`;
    const imageUrl = event.initialMedia?.[0]?.url || "https://via.placeholder.com/600x200"; // Image par défaut
    const formattedDate = new Date(event.details.date).toLocaleDateString(undefined, {
        weekday: "long",
        year: "numeric",
        month: "long",
        day: "numeric",
    ...
    const content = `
    <p style="font-size: 16px; color: #333;">Hi ${guest.username || "Guest"},</p>
    <p style="font-size: 14px; color: #333;">
      You are invited to ${isCoHost ? "co-host" : "join"} the event hosted by <strong>${user.username || "Evento"}</strong>.
    </p>
    <ul style="list-style-type: none; padding: 0; font-size: 14px; color: #333;">
      ...
      <li>📍 <strong>Location:</strong> <a href="https://maps.google.com/?q=${encodeURIComponent(event.details.location)}" target="_blank" style="color: #5f6fed; text-decoration: none;">
  ...
  `;
    ...
    const emailContent = wrapWithTemplate(header, content, imageUrl, guest.unsubscribeToken);
...
const sendAnnouncementEmail = async (user, recipient, event, announcement) => {
    if (!user || !recipient || !event || !announcement) {
        console.error("Missing required data for sending the email.");
        return;
    }
    const imageUrl = event.initialMedia?.[0]?.url || "https://via.placeholder.com/600x200";
                    ...
                    To: [
                        {
                            ...
                            Name: recipient.username || "Guest",
                        },
                    ],
                    Subject: subject,
                    HTMLPart: emailContent,
                    CustomID: "EventAnnouncement",
                },
            ],
        });
        const result = await request;
        console.log(`Announcement email sent to recipient:`, result.body);
    }
    catch (error) {
...
const sendEventReminderEmail = async (recipient, event) => {
    try {
        ...
        const coHostsPart = event.coHosts && event.coHosts.length > 0
            ? `& Co-hosted by ${event.coHosts.length === 1
                ...
                : event.coHosts.length === 2
                    ...
                    : `${event.coHosts
                        ...
                        .join(", ")}, and <strong>${event.coHosts[event.coHosts.length - 1].userId.username}</strong>`}`
            : "";
        // Génération de locationPart
        const locationPart = event.details.location
            ? `<li>📍<strong>Location:</strong> 
      <a href="https://www.google.com/maps/search/?api=1&query=${encodeURIComponent(event.details.location)}" target="_blank" style="color: #5f6fed; text-decoration: none;">${event.details.location}</a>
      <br />
      <span style="font-size: 12px; color: #555;">(If the link doesn't work, copy this address: <strong>${event.details.location}</strong>)</span>
    </li>`
            : "";
      ...
      <p>This is a reminder for the event: <strong>${event.title}</strong>, hosted by 
      <strong>${event.user.username}</strong> ${coHostsPart}, happening tomorrow at 
      ...
      <div style="text-align: center; margin-top: 20px;">
        <div style="max-width: 100%; height: 200px; overflow: hidden; border-radius: 10px;">
          <img src="${event.initialMedia?.[0]?.url || ""}" alt="event-image" style="width: 100%; height: 100%; object-fit: cover;" />
        </div>
      </div>
      <div style="text-align: center; margin-top: 20px;">
        <a href="https://www.evento-app.io/events/${event._id}" target="_blank" 
    ...
    `;
        ...
        const emailContent = wrapWithTemplate(header, content, "", recipient.unsubscribeToken);
        const client = getMailjetClient();
        if (!client)
            return;
        // Envoi de l'email
        ...
        const result = await request;
        console.log(`Event reminder email sent to ${recipient.email}:`, result.body);
    }
    catch (error) {
        console.error(`Error sending event reminder email to ${recipient.email}:`, error);
    }
};
const sendUpdateNotification = async (users, event, changeType, oldData = {}) => {
    try {
        ...
        `;
            ...
            const emailContent = wrapWithTemplate(header, content, imageUrl, user.unsubscribeToken);
            const client = getMailjetClient();
            if (!client)
                return;
            // Envoi de l'email
...
function buildChangeDetails(changeType, oldData, event) {
    ...
    switch (changeType) {
        ...
        case "date":
            ...
            const previousStartTime = highlightChanges(formatTimeOnly(oldData.startTime), formatTimeOnly(event.details.startTime), "previous");
            const newStartTime = highlightChanges(formatTimeOnly(oldData.startTime), formatTimeOnly(event.details.startTime), "new");
            const previousEndTime = oldData.endTime &&
                highlightChanges(formatTimeOnly(oldData.endTime), formatTimeOnly(event.details.endTime), "previous");
            const newEndTime = event.details.endTime &&
                highlightChanges(formatTimeOnly(oldData.endTime), formatTimeOnly(event.details.endTime), "new");
            const previousTimeZone = highlightChanges(oldData.timeZone, event.details.timeZone, "previous");
            const newTimeZone = highlightChanges(oldData.timeZone, event.details.timeZone, "new");
            // Construction des lignes
            const previousDateLine = previousStartDate === previousEndDate
                ? `${highlightChanges(previousStartDate, newStartDate, "previous")} ${previousEndTime
                    ? `from ${previousStartTime} to ${previousEndTime}`
                    : `at ${previousStartTime}`} (GMT ${previousTimeZone})`
                : `${highlightChanges(previousStartDate, newStartDate, "previous")} to ${highlightChanges(previousEndDate, newEndDate, "previous")} ${previousEndTime
                    ? `from ${previousStartTime} to ${previousEndTime}`
                    : `at ${previousStartTime}`} (GMT ${previousTimeZone})`;
            const newDateLine = newStartDate === newEndDate
                ? `${highlightChanges(previousStartDate, newStartDate, "new")} ${newEndTime
                    ? `from ${newStartTime} to ${newEndTime}`
                    : `at ${newStartTime}`} (GMT ${newTimeZone})`
                : `${highlightChanges(previousStartDate, newStartDate, "new")} to ${highlightChanges(previousEndDate, newEndDate, "new")} ${newEndTime
                    ? `from ${newStartTime} to ${newEndTime}`
                    : `at ${newStartTime}`} (GMT ${newTimeZone})`;
